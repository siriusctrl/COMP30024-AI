...
def cal_all(current_board, next_bor, colour, colour_e, colour_p, action, arrange, exit_this=False):
        ...
        if exit_this:
            ...
            pass
        log_uti = get_utility(current_board, next_bor, colour, d_heurii, colour_e, arrange, colour_p)
...
def get_utility(current_board, suc_bo, colour, re, colour_ea, arrange, colour_p):
    ...
    utility += cal_heuristic(suc_bo, colour, colour_ea, arrange, colour_p)
...
def cal_heuristic(suc_bo, colour, colour_exit, arrange, colour_p):

    heuris = [heuristic([x for x in suc_bo.keys() if suc_bo[x] == c],c,colour_exit[c], colour_p) for c in arrange]
    return heuris
...
def cal_rheu( cur_state, next_state, colour, player_exit, colour_p):
    ...
    cur_heuri = heuristic(cur_pl, colour, player_exit, colour_p)
    nxt_heuri = heuristic(nxt_pl, colour, player_exit, colour_p)
...
def hard_code_eva_function(pieces_difference: int, reduced_heuristic: float, danger_pieces: int, players, player_exit, action, other_rheu, colour, colour_p) -> float:
    ...
    if t < 4:
        res += 300 * pieces_difference + (-1) * reduced_heuristic + danger_pieces * (-20) + others
    elif t == 4:
        res += 50 * pieces_difference + (-10) * reduced_heuristic + danger_pieces * (-20) + others
    else:
        res += 50 * pieces_difference + (-10) * reduced_heuristic + danger_pieces * (-5) + 1.5 * others
...
def cal_otherrheu(cur_state, next_state, colour, colour_exit, colour_p):
    ...
    for c in colours:
        c_rh[c] = cal_rheu(cur_state, next_state, c, colour_exit[c], colour_p)
