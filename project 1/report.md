...
- State
  - Each node contains a state, different node contains different state. How board looks like described a state, every times the board changed, we regard it as a new state.
- Actions
...
- Action cost
  - Each action cost 1 (MOVE, JUMP, EXIT)
...
We made up of 3 test cases to test how good is our algorithm from 3 perspective.
...
- How good is our heuristic and frontier update strategy
  ...
  - This problem could represent one kind of question which has multiple pieces that can freely move around without blocks.